...
class SMSServerHandler(socketserver.BaseRequestHandler):
    ...
    def handle(self):
        ...
        for gateway in gateways:
            if not SETTINGS.modem_disable_check:
                try:
        ...
        if not success:
            return self.handel_error(f"No gateway with active state available.")

        self.request.sendall(SETTINGS.sms_success_message.encode(SETTINGS.sms_encoding))
    ...
    def handel_error(self, message):
        LOG.error(message)
        self.request.sendall(message.encode(SETTINGS.sms_encoding))
